package com.pedrofrohmut.todos.integration.web.controllers.users;
...
class UserControllerCreateIntTests {
  ...
  public UserControllerCreateIntTests() {
    connection = ConnectionFactory.getTestConnection();
    userDataAccessUtil = new UserDataAccessUtil(connection);
    userDataAccess = new UserDataAccessImpl(connection);
    passwordService = new BcryptPasswordService();
    jwtService = new JjwtJwtService();
    createUserUseCase = new CreateUserUseCase(userDataAccess, passwordService);
  ...
  void emailAlreadyRegistered() {
    ...
    final var passwordHash = passwordService.hashPassword("registered_user_password");
    final var registeredUser =
      new User(UUID.randomUUID().toString(), "Registered User Name", USER_EMAIL, passwordHash);
    userDataAccessUtil.create(registeredUser);
